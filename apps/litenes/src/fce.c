...
static int skip_frame = 2;
static int wait_start = 5;
static int wait_end = 20;
...
void fce_update_screen() {
  ...
  switch (state) {
    ...
    case STATE_WAIT_FIRST:
      now = uptime();
      if (now > wait_start * 1000) state = STATE_SHOW_FIRST;
      return;
    case STATE_SHOW_FIRST:
      state = STATE_WAIT_START;
      goto show;
    case STATE_WAIT_START:
      now = uptime();
      if (now > wait_end * 1000) state = STATE_START;
      return;
  }
  ...
  do_update = (frame_cnt == 0);
  frame_cnt ++;
  if (frame_cnt != skip_frame) return;
  frame_cnt = -1;
...
int main(int argc, char *argv[]) {
  ...
  if (argc >= 2) {
    wait_start = atoi(argv[1]);
  }
  if (argc >= 3) {
    wait_end = atoi(argv[2]);
  }
  if (argc >= 4) {
    skip_frame = atoi(argv[3]);
  }
